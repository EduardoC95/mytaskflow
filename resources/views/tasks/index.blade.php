...
@section('content')
    ...
    <div class="mb-6 rounded-2xl border border-[#c5ad88] bg-[#f6ead2]/90 p-5 shadow">
        <form method="GET" action="{{ route('tasks.index') }}" class="grid gap-4 md:grid-cols-4">
            <div>
                <label class="mb-2 block text-sm font-semibold text-[#4a3423]">Estado</label>
                <select name="status"
                        class="w-full rounded-xl border border-[#c5ad88] bg-[#fff8e8] px-4 py-3 text-[#3b2a1d] focus:border-[#8a5a32] focus:ring-2 focus:ring-[#c5ad88]/50">
            ...
            <div>
                <label class="mb-2 block text-sm font-semibold text-[#4a3423]">Prioridade</label>
            ...
            <div>
                <label class="mb-2 block text-sm font-semibold text-[#4a3423]">Data</label>
            ...
            <div class="flex items-end gap-2">
                <button type="submit"
                        class="w-full rounded-xl bg-[#4f565f] px-4 py-3 text-sm font-bold text-[#f6ead2] hover:bg-[#3f4650]">
                    Filtrar
                </button>

                <a href="{{ route('tasks.index') }}"
                   class="rounded-xl border border-[#c5ad88] bg-[#fff8e8] px-4 py-3 text-sm font-bold text-[#4a3423] hover:bg-[#f6ead2]">
                    Limpar
                </a>
            </div>
        </form>
    ...
    @if($tasks->count())
        <div class="space-y-5">
            @foreach($tasks as $task)
                <article class="group flex min-h-[220px] flex-col justify-between rounded-2xl border border-[#c5ad88] bg-[#f6ead2]/90 p-6 shadow transition-all duration-300 ease-out hover:-translate-y-1 hover:border-[#b99664] hover:bg-[#fff4dc] hover:shadow-xl">

                    <!-- TOP -->
                    <div>
                        <div class="mb-3 flex flex-wrap items-center gap-2">
                            <h2 class="text-xl font-bold text-[#3b2a1d]">
                                {{ $task->title }}
                            </h2>
                            ...
                            <span class="rounded-full px-3 py-1 text-xs font-bold
                                {{ $task->status === 'completed'
                                    ? 'bg-[#4f6f3a]/15 text-[#3f5c2f] ring-1 ring-[#4f6f3a]/30'
                                    : 'bg-[#4f565f]/15 text-[#4f565f] ring-1 ring-[#4f565f]/30' }}">
                                {{ $task->status === 'completed' ? 'Concluída' : 'Pendente' }}
                            </span>
                        </div>

                        <p class="mb-4 text-sm leading-6 text-[#6f4b2a]">
                    ...
                    <!-- ACTIONS (SEMPRE EM BAIXO) -->
                    <div class="mt-6 flex flex-wrap gap-2">
                        <a href="{{ route('tasks.show', $task) }}"
                           class="rounded-xl border border-[#c5ad88] bg-[#fff8e8] px-4 py-2 text-sm font-semibold text-[#4a3423] transition hover:-translate-y-0.5 hover:bg-[#f6ead2] hover:shadow">
                            Ver
                        </a>

                        <a href="{{ route('tasks.edit', $task) }}"
                           class="rounded-xl border border-[#c5ad88] bg-[#ead8b8] px-4 py-2 text-sm font-semibold text-[#4a3423] transition hover:-translate-y-0.5 hover:bg-[#f6ead2] hover:shadow">
                            Editar
                        </a>
                        ...
                        <form action="{{ route('tasks.toggle', $task) }}" method="POST">
                            ...
                            @method('PATCH')
                            <button type="submit"
                                    class="rounded-xl bg-[#4f6f3a] px-4 py-2 text-sm font-bold text-[#fff8e8] transition hover:-translate-y-0.5 hover:bg-[#3f5c2f] hover:shadow">
                        ...
                        <form action="{{ route('tasks.destroy', $task) }}" method="POST"
                            ...
                            @method('DELETE')
                            <button type="submit"
                                    class="rounded-xl bg-[#8b2f2f] px-4 py-2 text-sm font-bold text-[#fff8e8] transition hover:-translate-y-0.5 hover:bg-[#742626] hover:shadow">
                                Apagar
                            </button>
                        </form>
                    </div>
